...
GatesNotes uses a clean sans-serif font for both headings and body text. The body text is set at approximately 19px with generous line spacing. The overall impression is modern, clean, and highly readable.

*[These values are starting points derived from GatesNotes. Edit to match your preferences after consulting a design LLM.]*

| Element | Suggested approach | Notes |
|---------|-------------------|-------|
| Body text | 18-20px, sans-serif or serif | Serif feels literary and warm; sans-serif feels modern and clean. GatesNotes uses sans-serif. Choose based on the personality you want. |
| Line height | 1.6-1.7 × font size | GatesNotes uses generous leading. This is the single most important readability setting — do not go below 1.5. |
| Paragraph spacing | 1.2-1.5em between paragraphs | Use space between paragraphs, not first-line indents. Matches web reading conventions. |
| Article title | 32-40px, bold | Large enough to anchor the page, not so large it overwhelms. GatesNotes uses bold, large titles. |
| H2 in article | 24-28px, bold | Clear section break. Extra whitespace above (1.5-2em) signals a new topic. |
| H3 in article | 20-22px, semi-bold | Subtle section break. Less whitespace above than H2. |
| Navigation links | 14-16px, normal weight | Quiet and unobtrusive. |
| Date/metadata | 14px, normal weight, muted color | Informational, not prominent. |
| Captions | 14px, normal weight, muted color | Below images. Should not compete with body text. |

**Font loading:** If using a web font (Google Fonts or self-hosted), load with `font-display: swap` so text renders immediately with a system font fallback, then swaps when the web font loads. If using system fonts only, no loading strategy needed.

**Text color:** Body text should not be pure black (#000000) on pure white (#FFFFFF). GatesNotes uses a dark gray on a white or near-white background. A soft contrast like #222222 or #333333 on #FFFFFF, or #111111 on #FAFAFA, reduces eye strain for sustained reading.

**Text alignment:** Always left-aligned. Never justified — justified text creates uneven word spacing on the web where hyphenation is inconsistent.

### 2.4 Color Palette

GatesNotes uses a predominantly white/light palette with minimal accent color. The overall impression is restrained and sophisticated — color is used for links and occasional UI elements, not for decoration.

*[These are placeholder values. Fill in after design consultation.]*

| Role | Value | Notes |
|------|-------|-------|
| Page background | Off-white or warm light gray (e.g., #FAFAFA, #F5F5F0) | Pure white can feel clinical. A slight warmth feels more personal. |
| Content background | Same as page, or white (#FFFFFF) for article cards | Cards can be slightly lighter than the page to create subtle lift. |
| Body text | Dark gray (#222222 or #333333) | Not pure black. |
| Muted text | Medium gray (#777777 or #888888) | For dates, captions, metadata. |
| Headings | Same as body text, or very slightly darker | Headings get their emphasis from size and weight, not color. |
| Links | A single accent color (e.g., muted blue, teal, or warm tone) | Must have sufficient contrast against the background for accessibility. GatesNotes uses a blue. |
| Link hover | Darker shade of link color, or underline appears | Hover state should be subtle but noticeable. |
| Card borders/shadows | Very subtle — light gray border or soft box-shadow | GatesNotes uses subtle shadows on cards. Should not be heavy or dark. |
| Accent color | One color only, used sparingly | For links, active filter tags, and possibly the site name. Not for backgrounds or large areas. |

**Dark mode:** *[DECISION NEEDED. GatesNotes does not offer dark mode. Adding it roughly doubles the CSS color work. Recommendation: skip for launch, add later if readers request it.]*
...
- **Sticky behavior:** Use a sticky header that remains visible while scrolling. 
- On mobile: navigation links collapse into a hamburger menu icon (three horizontal lines). Tapping it reveals the links as a dropdown, slide-in panel, or full-screen overlay.
...
- Consistent card height within each row, or masonry-style variable height. *[DECISION NEEDED.]*

**Filter controls:**

If tag filtering is implemented on the home page (see 1.8), the filter UI should be:
- A row of tag pills or text links above the article grid.
- Tapping a tag filters the list to show only articles with that tag. Tapping again (or tapping "All") removes the filter.
- Active tag is visually highlighted (e.g., filled background in the accent color, or bold text).
- Filters should not reload the page — use JavaScript to show/hide cards, or use URL parameters if JS-free is preferred.
...
- *[OPTIONAL: Previous/Next article links. DECISION NEEDED.]*
...
- Navigation may remain visible or collapse to hamburger. *[DECISION NEEDED — GatesNotes collapses on tablet.]*
...
1. Sticky header, scroll-away header, or non-sticky header? THE HEADER SHOULD STAY IN PLACE WHEN THE USER SCROLLS DOWN OR UP. IT APPEARS OVER THE CONTENT, A FEW PIXELS BELOW THE TOP OF THE SCREEN.
2. Home page card grid: consistent card height or masonry-style variable height? THE HEIGHT SHOULD BE CONSISTENT, WITH A 3 COLUMN LAYOUT (LIKE GATESNOTES) WHEN DISPLAYED ON A LARGE MONITOR. ARTICLES SHOULD FEATURE A VARIETY OF 1, 2, AND 3 COLUMN WIDE POSTS. 
3. Body font: serif or sans-serif? Specific font choice? See Article Page and Home page sections
4. Heading font: same family as body or contrasting?
5. Dark mode: include at launch or defer? NO DARK MODE
6. Navigation on tablet: visible or collapsed to hamburger? As on GatesNotes, the header bar should collapse to a hamburger menu on narrow devices.
7. Previous/Next article links at bottom of article page? No.
8. Full color palette values (all TBD entries in 2.4). SHOULD MATCH GATESNOTES.
9. Full typography values (all TBD entries in 2.3). SHOULD MATCH GATESNOTES. WE CAN TWEEK WHEN WE SEE IT.
10. Tag filter behavior: JavaScript show/hide or URL-parameter page reload?
11. Article metadata format: SEPARATE JSON FILE. I WILL INCLUDE META DATA IN MD FILE, AND MANAGEMENT PROGRAM WILL GENERATE MANIFEST JSON.
