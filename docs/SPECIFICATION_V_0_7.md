...
On tiles for posts that contain a "hero image", the image will fill the tile. The Overline, Headline, and Summary will appear in a text overlay box.

**Text overlay width rule:** The text overlay box is always the width of one grid column, regardless of the tile's rendered column span. A 3-column tile has a 1/3-width text box. A 2-column tile has a 1/2-width text box. When the tile renders at 1 column (mobile, or tablet), the text box fills the full tile width.
...
### 1.4 Article Page
The main or hero image for the article will appear at the top of the Article Page. The image should break out of the text column and span up to approximately 1200px wide, centered. This is wider than both the text column (~700px) and the home page grid (~1100px), creating a three-tier width system. On viewports narrower than 1200px, the image fills the full viewport width.
