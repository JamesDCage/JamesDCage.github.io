...
### 1.3 Home Page  
The default landing page, showing all available articles as tiles. The tiles should be displayed in the order that they appear in `manifest.json`, with the first article in the JSON file as the first article on the site. This will be an "infinite scroll", with on-demand loading of new articles as the user scrolls to the bottom of the list. 

When displayed on a large monitor, the home page will consist of a 3-column grid. Every row except the last must sum to exactly 3 columns — no gaps. It is okay to have empty slots in the last row. The tile layout follows a fixed repeating row pattern, applied automatically from newest to oldest:
...
This pattern consumes 8 articles per cycle, then repeats. Articles follow the order that they appear in `manifest.json`.

Every post (article and video) must render correctly at any width (1, 2, or 3 columns). All tiles in a row are the same height. Rows that feature a 3-column post are tall enough to maintain a 16x9 aspect ratio for the row.  

**Tile height for small screens:** Tile height for small screens follow the rules for rows with 1-column wide content above. Once the viewport narrows enough so that only one tile per row is visible, all rows should be the same height.

**Featured tile (Row 1 of each cycle):** The first tile in each cycle is wider and taller than standard tiles, but does not fill the entire screen. It fills the full width of the white content area and is tall enough to fit an image with 16x9 proportions, passing behind the floating header. 
...
├── docs/                # Defining files for project, including this document
...
| 0.10 | 2026-04-29 | Minor housekeeping: updated date, clarified docs/ directory in structure. No spec changes. |
| 0.11 | 2026-05-17 | Manifest-based ordering (1.3): tiles now display in manifest.json order, not reverse chronological. Grid layout updated: last row may have empty slots. Featured tile and 3-column rows now maintain 16:9 aspect ratio. Simplified featured tile description. Updated small-screen tile height rules. |